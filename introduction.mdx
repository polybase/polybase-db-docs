...
Polybase is better than using a centralized database like Firebase, Postgres or Supabase because it has built-in web3 permissions. You can do row-level token gating and wallet-based permissions. Additionall, you can encrypt data using wallets for "self sovereign data" and verifiably query Polybase from smart collections (coming soon).
