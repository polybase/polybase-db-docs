...
You can use any authentication provider with the Polybase DB network. You simply need to provide a `signer` function 
to the Polybase DB client. Signing a request populates the [`ctx.publicKey`](/guides/collections#context) variable inside your collection code, allowing you to write code that 
...
To make authentication easier, Polybase DB provides an authentication library that allows users to sign in to 
...
<img src="/db/docs/images/polybase-auth-login.png" />
...
Ethereum Personal Sign is the signing mechanism used by Polybase DB (and many other dapps) to authenticate ownership
...
If you're using React, you can use our [React library `@polybase/react`](/guides/react) to help with control authentication.
