...
You must [define a collection](/guides/collections) before writing data to Polybase DB.
...
All data on Polybase DB is publicly accessible (like a blockchain). Therefore it is
...
safe, which could be either stored locally in browser storage or in Polybase DB.
...
You could store the encrypted private key on Polybase DB, this allow the encrypted
...
![Multi User Encryption](/db/docs/images/multi-user-encryption.png)
