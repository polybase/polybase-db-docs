...
const recordData = await collectionReference.create([userName, {email: userEmail, phone: userPhone}]) // DOES NOT WORK

const recordData = await collectionReference.create([userName, {email: userEmail, phone: ""}]) // WORKS

const recordData = await collectionReference.create([userName, {}]) // WORKS
