...
### How do write and read permissions work on Polybase?

Writes to Polybase need to be signed as mentioned above. The signature keys will be checked in the Polylang collection definition to accept or reject the transaction (using `ctx.publicKey`). Currently reads are public. We are close to releasing "private data" which could require any arbitrary read permissions (session key sig, wallet sig, token gating, etc). Our timeline to release this is sometime in March 2023.
